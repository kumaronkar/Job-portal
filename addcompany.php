...
if(isset($_POST)) {
	...
	$type = mysqli_real_escape_string($conn, 2);
	...
	if($result->num_rows == 0) {
		...
		if(file_exists($_FILES['image']['tmp_name'])) { 
			...
			if($imageFileType == "jpg" || $imageFileType == "png")  {
				...
				//Next we need to check file size with our limit size. I have set the limit size to 5MB. Note if you set higher than 2MB then you must change your php.ini configuration and change upload_max_filesize and restart  server
		...
		$sql = "INSERT INTO company(name, companyname, country, state, city, contactno, website, email, password, aboutme, logo) VALUES ('$name', '$companyname', '$country', '$state', '$city', '$contactno', '$website', '$email', '$password', '$aboutme', '$file','$type')";
